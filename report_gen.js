...
async function _buildReport(setStatus) {
  ...
  parents.forEach(sku => {
    ...
    if (!soldSkus.has(sku)) return; // ★ 선택월 판매 품목만
  ...
  // ══════════════════════════════════════════════════════
  // SLIDE HELPERS — 경계바 제거, 여백만 사용
  // ══════════════════════════════════════════════════════

  function addInsights(sl, insights, yPos) {
    const y = yPos || 4.3;
    sl.addText([
      { text: '💡 Insights', options: { bold: true, fontSize: 11, breakLine: true, color: C.accent } },
      ...insights.map((t, i) => ({ text: t, options: { bullet: true, breakLine: i < insights.length - 1, fontSize: 10 } }))
    ], { x: 0.5, y: y + 0.05, w: 9, h: 1.2, color: C.dark, valign: 'top' });
  }
  ...
  s3.addChart(pres.charts.DOUGHNUT, [{
    ...
    x: 5.6, y: 0.7, w: 4.0, h: 3.2, showTitle: false,
  ...
  s4.addText(`⚠️ 재고부족 품목 — ${cntLow}건`, { x: 0.4, y: 0.85, w: 5, h: 0.3, fontSize: 11, bold: true, color: C.red, margin: 0 });
  ...
  if (riskRows.length > 0) {
    s4.addTable([
      ...
      ...riskRows
    ], { x: 0.4, y: 1.15, w: 5.5, fontSize: 10, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [2.8, 1.2, 1.5], autoPage: false });
  }
  ...
  s4.addTable([
    ...
    ...catRows
  ], { x: 6.1, y: 1.15, w: 3.5, fontSize: 10, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [1.4, 0.9, 1.2], autoPage: false });

  if (excessItems.length > 0) {
    const exTop = excessItems.slice(0, 4).map(i => `${_nm(i.sku_id)} (현재고 ${_f(getEq(i.sku_id))}개, 최근출고 ${_f(_recentOut(i.sku_id, 3))}개)`);
    const exY = 1.15 + (riskRows.length + 1) * 0.32 + 0.25;
    s4.addText([
    ...
    ], { x: 0.4, y: exY, w: 9.2, h: 1.5, color: C.dark, valign: 'top' });
  }
  ...
  s6.addTable([
    ...
    ...itemRows
  ], { x: 0.3, y: 1.0, w: 9.4, fontSize: 9.5, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [0.4, 3.5, 0.9, 1.2, 0.8, 1.2], autoPage: false });
  ...
  s7.addText(`${periodStr} 판매 ${costItems.length}개 품목 | 평균 원가율 ${avgCR}%`, { x: 4.0, y: 0.3, w: 5.5, h: 0.35, fontSize: 11, color: C.sub, align: 'right' });
  ...
  ], { x: 5.3, y: 0.9, w: 4.3, h: 2.6, fontSize: 11, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [2.0, 2.3], autoPage: false });
  ...
  // ══════════════════════════════════════════════════════
  // SLIDE 8: 원가 상세 — 선택월 판매 품목 원가율 & 마진
  // ══════════════════════════════════════════════════════
  setStatus('원가 상세...', 55);
  const s8 = pres.addSlide();
  s8.background = { color: C.white };
  s8.addText(`💹 원가 상세 — ${periodStr} 판매 품목`, { x: 0.5, y: 0.25, w: 9, h: 0.55, fontSize: 20, fontFace: 'Arial Black', color: C.dark, margin: 0 });
  s8.addText('원가율 높은 순', { x: 0.5, y: 0.72, w: 9, h: 0.25, fontSize: 11, color: C.sub });
  ...
  s8.addTable([
    [{ text: '품목명', options: { bold: true, fill: { color: C.navy }, color: C.white } },
     ...
     { text: '당월매출', options: { bold: true, fill: { color: C.navy }, color: C.white, align: 'right' } }],
    ...crRows
  ], { x: 0.3, y: 0.95, w: 9.4, fontSize: 9.5, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [3.0, 1.2, 1.2, 1.0, 1.5, 1.5], autoPage: false });

  // ══════════════════════════════════════════════════════
  // SLIDE 9: 고위험 원가 상세 (>70%)
  // ══════════════════════════════════════════════════════
  if (highCR.length > 0) {
    setStatus('고위험 원가 상세...', 60);
    const s9 = pres.addSlide();
    s9.background = { color: C.white };
    s9.addText('⚠️ 고위험 원가 상세 (원가율 70% 초과)', { x: 0.5, y: 0.25, w: 9, h: 0.55, fontSize: 20, fontFace: 'Arial Black', color: C.red, margin: 0 });
    s9.addText(`${periodStr} 판매 품목 중 ${highCR.length}건`, { x: 0.5, y: 0.72, w: 9, h: 0.25, fontSize: 11, color: C.sub });
    ...
    const hrRows = highCR.slice(0, 10).map((c, i) => {
      ...
      const topMatCost = topMat ? (D.inv.find(x => x.sku_id === topMat.child_sku) || {}).unit_cost || 0 : 0;
      return [
        String(i + 1),
        c.name,
    ...
    s9.addTable([
      [{ text: '#', options: { bold: true, fill: { color: C.red }, color: C.white } },
       { text: '품목명', options: { bold: true, fill: { color: C.red }, color: C.white } },
       { text: '원가율', options: { bold: true, fill: { color: C.red }, color: C.white, align: 'right' } },
       { text: '판매가', options: { bold: true, fill: { color: C.red }, color: C.white, align: 'right' } },
       { text: '원가', options: { bold: true, fill: { color: C.red }, color: C.white, align: 'right' } },
       { text: '마진', options: { bold: true, fill: { color: C.red }, color: C.white, align: 'right' } },
       { text: '최대 원가 자재', options: { bold: true, fill: { color: C.red }, color: C.white } }],
      ...hrRows
    ], { x: 0.3, y: 1.0, w: 9.4, fontSize: 9, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [0.35, 2.4, 0.8, 1.1, 1.1, 1.1, 2.55], autoPage: false });
    ...
    addInsights(s9, [
      ...
      `최대 원가 자재 컬럼 활용하여 BOM 원가 최적화 검토`
    ], 1.0 + (hrRows.length + 1) * 0.32 + 0.15);
  }
  ...
  sBom.background = { color: C.white };
  sBom.addText('🔧 BOM 관리 & 생산가능성', { x: 0.5, y: 0.25, w: 9, h: 0.55, fontSize: 24, fontFace: 'Arial Black', color: C.dark, margin: 0 });
  ...
  ], { x: 0.4, y: 0.9, w: 4.5, h: 2.8, fontSize: 11, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [2.5, 2.0], autoPage: false });
  ...
  sCust.background = { color: C.white };
  sCust.addText('🏭 거래처 분석 (판매)', { x: 0.5, y: 0.25, w: 6, h: 0.55, fontSize: 24, fontFace: 'Arial Black', color: C.dark, margin: 0 });
  sCust.addText(`TOP5 집중도 ${custConcentration}% | ${periodStr}`, { x: 5, y: 0.3, w: 4.5, h: 0.35, fontSize: 11, color: C.sub, align: 'right' });
  ...
  sCust.addTable([
    [{ text: '#', options: { bold: true, fill: { color: C.navy }, color: C.white } },
     ...
     { text: 'vs전월', options: { bold: true, fill: { color: C.navy }, color: C.white, align: 'right' } }],
    ...custRows
  ], { x: 0.3, y: 0.85, w: 9.4, fontSize: 9.5, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [0.4, 3.5, 1.8, 1.2, 2.5], autoPage: false });

  const custTblEnd = 0.85 + (custRows.length + 1) * 0.32;
  addInsights(sCust, [
  ...
  sTa.background = { color: C.white };
  sTa.addText('📑 타계정 내역', { x: 0.5, y: 0.25, w: 6, h: 0.55, fontSize: 24, fontFace: 'Arial Black', color: C.dark, margin: 0 });
  sTa.addText(`${periodStr} ${_m(taTotal)} / ${_f(taQty)}건  (${yr}년 누계 ${_m(taYrTotal)})`, { x: 4.0, y: 0.3, w: 5.5, h: 0.35, fontSize: 11, color: C.sub, align: 'right' });
  ...
  sTa.addText(`계정별 집계 (${periodStr})`, { x: 5.8, y: 0.9, w: 4, h: 0.3, fontSize: 11, bold: true, color: C.sub, margin: 0 });
  if (taAcctList.length > 0) {
    ...
    ], { x: 5.8, y: 1.2, w: 3.8, fontSize: 10, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [1.5, 0.9, 1.4], autoPage: false });
  }
  ...
  // ══════════════════════════════════════════════════════
  // SLIDE: 공급사 관리 (구매) + 구매 품목 TOP
  // ══════════════════════════════════════════════════════
  setStatus('공급사 관리...', 90);
  const sVen = pres.addSlide();
  sVen.background = { color: C.white };
  sVen.addText('🏭 공급사 관리 (구매)', { x: 0.5, y: 0.25, w: 6, h: 0.55, fontSize: 24, fontFace: 'Arial Black', color: C.dark, margin: 0 });
  sVen.addText(`${periodStr} | ${poVendors.length}개사 ${_m(poAmt)}${prevPOAmt > 0 ? ' (전월 ' + _chg(poAmt, prevPOAmt) + ')' : ''}`, { x: 4, y: 0.3, w: 5.5, h: 0.35, fontSize: 11, color: C.sub, align: 'right' });

  // 공급사 테이블 (좌)
  const vRows = topPOVendors.slice(0, 6).map(([nm, v]) => [
  ...
  sVen.addText('공급사별 발주', { x: 0.3, y: 0.85, w: 5, h: 0.25, fontSize: 11, bold: true, color: C.sub, margin: 0 });
  sVen.addTable([
    [{ text: '공급사', options: { bold: true, fill: { color: C.navy }, color: C.white } },
     ...
     { text: '비중', options: { bold: true, fill: { color: C.navy }, color: C.white, align: 'right' } }],
    ...vRows
  ], { x: 0.3, y: 1.1, w: 5.1, fontSize: 9.5, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [1.8, 0.8, 1.3, 1.2], autoPage: false });

  // 구매 품목 TOP (우)
  sVen.addText('구매 품목 상위', { x: 5.6, y: 0.85, w: 4, h: 0.25, fontSize: 11, bold: true, color: C.sub, margin: 0 });
  const poItemRows = topPOItems.slice(0, 8).map(([nm, v]) => [
    nm, { text: _f(v.qty), options: { align: 'right' } }, { text: _m(v.amt), options: { align: 'right' } }
  ]);
  sVen.addTable([
    [{ text: '품목명', options: { bold: true, fill: { color: C.purple }, color: C.white } },
     { text: '수량', options: { bold: true, fill: { color: C.purple }, color: C.white, align: 'right' } },
     { text: '금액', options: { bold: true, fill: { color: C.purple }, color: C.white, align: 'right' } }],
    ...poItemRows
  ], { x: 5.6, y: 1.1, w: 4.0, fontSize: 9, border: { type: 'solid', pt: 0.5, color: 'E2E8F0' }, colW: [1.8, 0.8, 1.4], autoPage: false });

  const venTblEnd = 1.1 + Math.max(vRows.length, poItemRows.length) * 0.3 + 0.4;
  addInsights(sVen, [
